...
- **内置规则库**：默认集成 SWC AST 静态分析规则，例如 `no-deep-relative-imports`、`no-circular-dependencies`、`no-swallowed-errors`、`no-cross-package-private-imports`、`no-catastrophic-regex`、`no-deep-clone-in-loop`、`no-n2-array-lookup`。
