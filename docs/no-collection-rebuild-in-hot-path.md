...
- 覆盖的“构建”形态：
  ...
  - `Object.keys` / `values` / `entries` / `fromEntries(x)`、`Array.from(x)`、`[...x]`。

## 3. 规则说明
- 约束：热路径中不得反复构建一个在迭代之间不变、且只被只读使用的集合。
- 判定方式（三个条件同时满足才上报）：
  - **只读消费**：构建结果立刻被查找/取长度/按下标读取（`.has` / `.get` / `.includes` / `.indexOf` / `.find` / `.some` / `.every` / `.at` / `.join` / `.length` / `.size` / `[i]`）；或赋给本轮的 `const`，且该常量在本作用域内只以上述方式被读取。被修改、被传参、被返回、被 `for...of` 遍历的结果都不报——那可能是有意为每轮准备的新副本，遍历本身也是 O(m)，重建只多一个常数因子。
  - **迭代间不变**：表达式只由标识符、成员访问、字面量与上述构建调用组成；回调中没有 `await`、随机数或对外部变量的写入；引用到的名字不是循环变量/回调参数，没有在作用域内声明、赋值或被原地修改（调用了非只读方法也算修改），也没有在文件其它地方被重新赋值。若回调读取了元素属性（`(a) => a.id`），作用域内还不能有任何属性写入。
  - **不在 while / do...while 的条件里**：这类条件必须随迭代变化才能终止，据此推断“不变”并不可靠。
...
## 5. 例外/豁免
- 规则按“宁可漏报、不误报”设计：循环中调用的普通函数若悄悄修改了集合（例如 `refresh()` 内部 `admins.push(...)`），静态分析看不到，因此只在集合被直接赋值/原地修改/调用非只读方法时才认定为“会变化”。遇到这种隐式修改导致的误报，可在该处关闭本规则。
