...
const translations = {
  zh: {
    ...
    scanProgress: ({ percent, current, total }) => `Auto Code Review 扫描进度: ${percent}% (${current}/${total})`,
    scanComplete: () => '代码扫描完成',
  ...
  en: {
    ...
    scanProgress: ({ percent, current, total }) => `'Auto Code Review' scan progress: ${percent}% (${current}/${total})`,
